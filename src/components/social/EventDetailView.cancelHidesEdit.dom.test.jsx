...
// "Participer"/"Annuler l'événement" juste en dessous, déjà masqués par
// `!canceled`. Or EventEditForm.jsx modifie
...
//
// Second bug identifié lors du même audit (portant cette fois sur le flux
// d'annulation lui-même, pas seulement son interaction avec l'édition) :
// "Supprimer l'événement" était À L'ÉPOQUE imbriqué dans le même bloc
// `{!canceled && (...)}` que "Participer"/"Annuler l'événement" — alors que
// rien côté serveur (policies "Le createur supprime son evenement" / "Un
// admin supprime n'importe quel evenement", supabase-delete-own-content.sql)
// ne conditionne la suppression à canceled_at. Un événement annulé se
// retrouvait donc bloqué sans aucune issue : ni annulable une seconde fois
// (normal), ni modifiable (masqué à raison), ni supprimable — pour son
// créateur ni pour un admin plateforme. Le bouton Supprimer est maintenant
// sorti de ce bloc pour rester disponible même après annulation.
...
describe("EventDetailView — bouton Modifier après annulation", () => {
  ...
  it("masque le bouton Modifier une fois l'événement annulé, même pour un organisateur", () => {
    ...
    // "Annuler l'événement" reste bien masqué (déjà annulé, pas de double
    // annulation possible), contrairement à "Supprimer" ci-dessous.
    expect(screen.queryByRole("button", { name: /Annuler l'événement/ })).not.toBeInTheDocument();
  });

  it("garde le bouton Supprimer visible pour le créateur même après annulation, sans état bloqué", () => {
    renderDetail({ ...BASE_EVENT, canceled_at: new Date().toISOString() });
    expect(screen.getByRole("button", { name: /Supprimer l'événement/ })).toBeInTheDocument();
  });

  it("garde le bouton Supprimer (admin) visible pour un admin plateforme après annulation", () => {
    render(
      <ImageLightboxProvider>
        <EventDetailView
          event={{ ...BASE_EVENT, canceled_at: new Date().toISOString() }}
          viewerRole={null}
          viewerStatus={null}
          participantCount={1}
          mutualCount={0}
