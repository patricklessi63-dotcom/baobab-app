...
export default function LocationRequiredGate({ onRetry, retrying, onSignOut }) {
  return (
    <div className="min-h-screen flex items-center justify-center p-6" style={{ background: C.sand }}>
      <div className="bb-card p-8 max-w-sm w-full text-center">
        ...
        <h1 className="text-lg font-black" style={{ color: C.indigo }}>Localisation requise</h1>
        ...
        <button onClick={onRetry} disabled={retrying} className="w-full mt-6 py-3 rounded-full text-sm font-bold text-white disabled:opacity-60" style={{ background: C.indigo }}>
          {retrying ? "Vérification..." : "J'ai réactivé la localisation"}
        </button>
        <button onClick={onSignOut} className="w-full mt-3 py-3 rounded-full text-sm font-semibold" style={{ color: "rgba(var(--bb-ink-rgb-static),0.6)" }}>
          Se déconnecter
        </button>
      </div>
    </div>
