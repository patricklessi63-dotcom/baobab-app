...
| 5 | Expérience de matching | Double opt-in (like mutuel) ✓. Signalement/blocage désormais accessibles depuis plus de surfaces ✓. **Vérifié ce soir** : un profil signalé est bien exclu immédiatement des suggestions futures (`submitReport` insère dans `hidden_recommendations`, sans attendre la modération — App.jsx:749). Aucun flux séparé par type de lien (amitié/mentorat/pro/amoureux) — un seul flux généraliste. | **Partiel** | Exclusion sur signalement : **Atteint** (2026-08-25). Reste : types de lien multiples, chantier important non commencé. |
| 6 | Expérience de messagerie | Accusés de lecture ✓ (avec réglage de confidentialité), indicateur de saisie ✓, médias (photo/vidéo/vocal) ✓, réponse ciblée ✓ (bug de perte corrigé ce soir), réactions ✓, nudges anti-arnaque déjà implémentés (`moneyGuard.js`). Dégradation connexion instable ✓. **Vérifié ce soir** : recherche dans l'historique d'une conversation déjà présente (`ConversationPane.jsx`, icône loupe). Traduction automatique : absente. Recherche à travers **toutes** les conversations (pas juste celle ouverte) : absente. | **Partiel** | Traduction à la demande : chantier IA, dépend d'une clé API absente côté Supabase (voir item 10). Recherche globale multi-conversations : chantier non commencé, priorité basse. |
...
| 8 | Sécurité toujours accessible | Signalement/blocage présents sur profil public, conversation, participants d'événement, membres de communauté, **et désormais les Stories** (menu ⋮ ajouté ce soir, absent jusqu'ici — un statut n'avait aucun accès sécurité). Nudges anti-arnaque actifs en messagerie. | **Partiel** | Stories : **Atteint** (2026-08-25). Reste à vérifier : accès depuis le Fil (posts) — déjà présent via l'icône Signaler sur chaque publication ; transparence de la visibilité du profil à documenter. |
| 9 | Interaction légère entre utilisateurs | Réactions rapides sur publications ✓ (fil + communautés). **Vérifié ce soir** : commentaires sur événements (onglet Discussion) ✓, compteur de participants ✓ (`EventDetailView.jsx`). Seul manque : le recoupement « dont Y dans votre réseau » — nécessite une notion de « réseau »/connexions mutuelles qui n'existe pas clairement dans le modèle de données actuel (matches ≠ réseau social classique). | **Partiel** (proche de Atteint) | Recoupement réseau sur les événements : ambigu à définir (que compte comme « réseau » ?), à clarifier avec l'utilisateur avant de construire plutôt que de deviner. |
...
**1. Sécurité/incohérences (priorité absolue) — fait ce soir :**
- ✅ Exclusion immédiate d'un profil signalé des suggestions de matching futures : déjà en place, vérifié (item 5).
- ✅ Accès sécurité (Signaler/Bloquer) ajouté aux Stories, seule surface d'interaction qui n'en avait aucun (item 8).

**2. Gains rapides sur les « Partiel » — fait ce soir :**
- ✅ Recherche dans l'historique de conversation : déjà en place, vérifiée (item 6).
- ✅ Commentaires + compteur de participants sur événements : déjà en place, vérifiés (item 9).
